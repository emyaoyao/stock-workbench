...
def _proxy_alive():
    try:
        urllib.request.urlopen("https://fuyao.aicubes.cn/", timeout=6)
        return True
    except Exception:
        return False
if not _proxy_alive():
    ...
    print("[net] 本地代理不可用，已绕过直连。", flush=True)
else:
...
DATA = os.path.join(HERE, "workbench_data.json")
BAK = os.path.join(HERE, "workbench_data.json.bak")
...
def build_daily():
    ...
    for key, (label, _) in cats.items():
        ...
        strong = [to_sw_entry(P.build_board_entry(ts, nm, board_snaps[key].get(ts),
                                                 cons_map.get(ts, []), stock_snap), i + 1)
                  for i, (p, ts, nm) in enumerate(top)]
        weak = [to_sw_entry(P.build_board_entry(ts, nm, board_snaps[key].get(ts),
                                               cons_map.get(ts, []), stock_snap), i + 1)
                for i, (p, ts, nm) in enumerate(bottom)]
        sw[key] = {"strong": strong, "weak": weak}
        boards = []
        for i, (p, ts, nm) in enumerate(list(top) + list(bottom), 1):
            e = P.build_board_entry(ts, nm, board_snaps[key].get(ts),
